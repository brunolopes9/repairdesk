...
public sealed class AppointmentService : IAppointmentService
{
    ...
    {
        // Sprint 443: lógica iCalendar extraída para IcsBuilder, partilhada com o
        // endpoint público de subscrição (token-auth) de S443.
        var items = await _repo.ListByRangeAsync(fromUtc.ToUniversalTime(), toUtc.ToUniversalTime(), status, ct);
        var bytes = IcsBuilder.BuildCalendar(items);
        var stamp = IcsBuilder.DateRangeStamp(fromUtc, toUtc);
        return new AppointmentCalendarExport(bytes, $"mender-agendamentos_{stamp}.ics");
    }
