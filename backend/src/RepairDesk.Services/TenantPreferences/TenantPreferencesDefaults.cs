...
public static class TenantPreferencesDefaults
{
    ...
    public static Dictionary<string, WhatsAppStateTemplate> CreateWhatsAppTemplates()
    {
        return new Dictionary<string, WhatsAppStateTemplate>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["Pronto"] = new(true, "Ola {{cliente_nome}}, o teu {{equipamento}} ja esta pronto para levantamento na {{loja_nome}}. Podes passar quando der jeito dentro do nosso horario: {{horario_loja}}. Podes tambem acompanhar e pagar online aqui: {{portal_link}}", 60),
